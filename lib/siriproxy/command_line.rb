...
Options:
  ...
  def update(directory=nil)
    if(directory)
      ...
      puts "=== Installing latest code from git://github.com/128keaton/SiriProxy.git [#{@branch}] ==="

	  tmp_dir = "/tmp/SiriProxy.install." + (rand 9999).to_s.rjust(4, "0")

	  `mkdir -p #{tmp_dir}`
      puts `git clone #{branch_opt} git://github.com/128keaton/SiriProxy.git #{tmp_dir}`  if $?.exitstatus == 0
